...
static void ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiID tab_id, ImGuiTabBarSection* sections)
{
    ...
    ImGuiContext& g = *GImGui;
    float _margin = g.FontSize * 1.0f; // When to scroll to make Tab N+1 visible always make a bit of N visible to suggest more scrolling area (since we don't have a scrollbar)
    ...
    float tab_x1 = tab->Offset - sections[0].Width + (order > sections[0].TabCount - 1 ? -_margin : 0.0f);
    float tab_x2 = tab->Offset - sections[0].Width + tab->Width + (order + 1 < tab_bar->Tabs.Size - sections[2].TabCount ? _margin : 1.0f);
    tab_bar->ScrollingTargetDistToVisibility = 0.0f;
