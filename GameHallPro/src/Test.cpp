...
static bool buttonClickedOnce = false; // 用于跟踪按钮是否已被点击

static void ShowExampleWindow()
{

	ImGui::Begin("Example Window", nullptr);

	if (!buttonClickedOnce)
	{
		if (ImGui::Button("Click me once"))
		{
	...
	if (buttonClickedOnce)
	{
		...
		ImGui::SameLine();
		ImGui::TextDisabled("(not clickable)");
		ImGui::SameLine();
		ImGui::Button("Click me once", ImVec2(-1, 0)); // 这里禁用了按钮但仍然显示它
	}

	ImGui::End();
}

static void ShowExampleWindow2()
{
	ImGui::Begin("Example Window 2", nullptr);
